...
const PaddlerCircle: React.FC<{ paddler: Paddler; isDragging?: boolean }> = ({ paddler, isDragging }) => {
  ...
  return (
    <div 
      className={`relative flex-shrink-0 rounded-full flex items-center justify-center font-bold text-white 
    ...
    >
      {/* Name - centered in the main circle */}
      <span className="text-[10px] leading-tight text-center px-1 truncate max-w-full">
        {displayName}
      </span>
      
      {/* Ability badge - smaller circle positioned at bottom left INSIDE */}
      <div 
        className="absolute bottom-1 left-1 w-3.5 h-3.5 rounded-full flex items-center justify-center text-[6px] font-bold text-white border border-white/60"
        style={{ 
